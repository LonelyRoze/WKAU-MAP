...
function M.new(map, imageX, imageY, imgContX, imgContY, sceneGroup)
    ...
    function loadImage(map)
        ...
        imageContainer = display.newContainer(imageX, imageY)
    
        chartImage = display.newImageRect(map, imageX, imageY)
        imageContainer:insert(chartImage)
        imageContainer:addEventListener("touch", imageContainer)
    
        imageContainer.x = imgContX
        imageContainer.y = imgContY
            ...
            elseif self.isFocus then
                ...
                elseif "ended" == phase or "cancelled" == phase then
                    ...
                    if (#previousTouches > 0) then
                        ...
                        if (math.abs(startX - event.x) < 12) and (math.abs(startY - event.y) < 12) and (not isMulti) then submitTouch(event
                            .x, event.y) end
                        prevX = imageContainer.x
                        prevY = imageContainer.y
    ...
    loadImage(map)
    sceneGroup:insert(imageContainer)
end
